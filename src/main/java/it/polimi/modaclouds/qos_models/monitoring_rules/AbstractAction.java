...
public abstract class AbstractAction {
	...
	/**
	 * Checks if the action expressed in the rule exists and if all
	 * required parameters were specified. Then calls the action
	 * specific validation check, i.e., {@code validate()}.
	 * 
	 * @param rule
	 * @param otherRules
	 * @return the set of problems found during the validation, an empty
	 * set otherwise.
	 */
	...
	/**
	 *  
	 * @return the list of required parameters that the user should put 
	 * in the monitoring rule.
	 */
	protected abstract Set<String> getMyRequiredPars();

	/**
	 * Action specific validation checks can be implemented here. Note
	 * that basic validation on the name of the action and on the 
	 * required parameters is already done.
	 * 
	 * @param rule the rule for which the action should be validated
	 * @param otherRules all other rules already installed or about to be 
	...
	protected Set<Action> getMyActions(MonitoringRule rule) {
		...
		for (Action action : rule.getActions().getActions()) {
			if (action.getName().equalsIgnoreCase(getName())) {
				actions.add(action);
			}
		}
		return actions;
	...
	public static AbstractAction getActionInstance(Action action) {
		...
		Reflections reflections = new Reflections("it.polimi.modaclouds.qos_models.monitoring_rules.actions");
		...
		for (Class<? extends AbstractAction> actionClass : actionClasses) {
			if (actionClass.getSimpleName().equalsIgnoreCase(action.getName())){
				try {
	...
	/**
	 * The concrete action to be executed when the rule is evaluated and
